...
})(this, function (t, e, a, n, o, d, i) {
  'use strict';
  const M = { selection: e.selection, select: e.select, max: a.max, min: a.min, sum: a.sum, cumsum: a.cumsum, tree: n.tree, stratify: n.stratify, zoom: o.zoom, zoomIdentity: o.zoomIdentity, linkHorizontal: i.linkHorizontal };
  (t.OrgChart = class {
    constructor() {
      const a = {
        ...
        linkUpdate: function (t, e, a) {
          M.select(this)
            .attr('stroke', (t) => (t.data._upToTheRootHighlighted ? '#152785' : 'lightgray'))
            .attr('stroke-width', (t) => (t.data._upToTheRootHighlighted ? 5 : 2)),
            t.data._upToTheRootHighlighted && M.select(this).raise();
        },
        nodeUpdate: function (t, e, a) {
          M.select(this)
            .select('.node-rect')
            .attr('stroke', (t) => (t.data._highlighted || t.data._upToTheRootHighlighted ? '#152785' : 'none'))
        ...
        siblingsMargin: (t) => 20,
        childrenMargin: (t) => 60,
        neightbourMargin: (t, e) => 80,
        compactMarginPair: (t) => 100,
        compactMarginBetween: (t) => 20,
        onNodeClick: (t) => t,
        linkGroupArc: M.linkHorizontal()
          .x((t) => t.x)
          .y((t) => t.y),
        nodeContent: (t) => `<div style="padding:5px;font-size:10px;">Sample Node(id=${t.id}), override using <br/> <br/> 
            <code>chart<br/>
            &nbsp;.nodeContent({data}=>{ <br/>
        ...
        layoutBindings: {
          left: {
            ...
            nodeFlexSize: ({ height: t, width: e, siblingsMargin: a, childrenMargin: n, state: o, node: i }) => {
              return o.compact && i.flexCompactDim ? [i.flexCompactDim[0], i.flexCompactDim[1]] : [t + a, e + n];
            },
          ...
          top: {
            ...
            nodeFlexSize: ({ height: t, width: e, siblingsMargin: a, childrenMargin: n, state: o, node: i }) => {
              return o.compact && i.flexCompactDim ? [i.flexCompactDim[0], i.flexCompactDim[1]] : [e + a, t + n];
            },
    ...
    initializeEnterExitUpdatePattern() {
      M.selection.prototype.patternify = function (t) {
        var e = t.selector,
          a = t.tag,
          t = t.data || [e],
    ...
    initialZoom(t) {
      ...
      return (e.lastTransform.k = t), this;
    }
    render() {
      const i = this.getChartState();
      if (!i.data || 0 == i.data.length) return console.log('ORG CHART - Data is empty'), this;
      const t = M.select(i.container);
      var e = t.node().getBoundingClientRect();
      0 < e.width && (i.svgWidth = e.width);
      const a = { id: `ID${Math.floor(1e6 * Math.random())}`, chartWidth: i.svgWidth, chartHeight: i.svgHeight };
      if (((i.calc = a), (a.centerX = a.chartWidth / 2), (a.centerY = a.chartHeight / 2), i.firstDraw)) {
        const r = { zoom: null };
        (r.zoom = M.zoom()
          .on('zoom', (t, e) => this.zoomed(t, e))
          .scaleExtent(i.scaleExtent)),
          (i.zoomBehavior = r.zoom);
      }
      (i.flexTreeLayout = d
        .flextree({
          nodeSize: (t) => {
            var e = i.nodeWidth(t),
              a = i.nodeHeight(t),
              n = i.siblingsMargin(t),
              o = i.childrenMargin(t);
            return i.layoutBindings[i.layout].nodeFlexSize({ state: i, node: t, width: e, height: a, siblingsMargin: n, childrenMargin: o });
          },
        })
        .spacing((t, e) => (t.parent == e.parent ? 0 : i.neightbourMargin(t, e)))),
        this.setLayouts({ expandNodesFirst: !1 });
      const n = t.patternify({ tag: 'svg', selector: 'svg-chart-container' }).style('background-color', i.backgroundColor).attr('width', i.svgWidth).attr('height', i.svgHeight).attr('font-family', i.defaultFont);
      i.firstDraw && n.call(i.zoomBehavior).on('dblclick.zoom', null).attr('cursor', 'move'), (i.svg = n);
      const o = n.patternify({ tag: 'g', selector: 'chart' });
      return (
        (i.centerG = o.patternify({ tag: 'g', selector: 'center-group' })),
        (i.linksWrapper = i.centerG.patternify({ tag: 'g', selector: 'links-wrapper' })),
        (i.nodesWrapper = i.centerG.patternify({ tag: 'g', selector: 'nodes-wrapper' })),
        (i.connectionsWrapper = i.centerG.patternify({ tag: 'g', selector: 'connections-wrapper' })),
        (i.defsWrapper = n.patternify({ tag: 'g', selector: 'defs-wrapper' })),
        i.firstDraw && i.centerG.attr('transform', () => i.layoutBindings[i.layout].centerTransform({ centerX: a.centerX, centerY: a.centerY, scale: i.lastTransform.k, rootMargin: i.rootMargin, root: i.root, chartHeight: a.chartHeight, chartWidth: a.chartWidth })),
        (i.chart = o),
        this.update(i.root),
        M.select(window).on(`resize.${i.id}`, () => {
          var t = M.select(i.container).node().getBoundingClientRect();
          i.svg.attr('width', t.width);
        }),
        i.firstDraw && (i.firstDraw = !1),
        this
      );
    }
    addNode(e) {
    ...
    removeNode(e) {
      const a = this.getChartState(),
        t = a.allNodes.filter(({ data: t }) => a.nodeId(t) == e)[0];
      if (!t) return console.log(`ORG CHART - REMOVE - Node with id "${e}" not found in the tree`), this;
      t.descendants().forEach((t) => (t.data._filteredOut = !0));
      const n = this.getNodeChildren(t, [], a.nodeId);
      n.forEach((t) => (t._filtered = !0)), (a.data = a.data.filter((t) => !t._filtered));
      const o = this.updateNodesState.bind(this);
      return o(), this;
    }
    groupBy(t, a, e) {
      const n = {};
      ...
      t.eachBefore((t) => {
        ...
        t.eachBefore((t) => {
          if (t.children && 1 < t.children.length) {
            ...
            if (!(n.length < 2)) {
              n.forEach((t, e) => {
                e || (t.firstCompact = !0), (t.compactEven = !(e % 2)), (t.row = Math.floor(e / 2));
              });
              var e = M.max(
                  n.filter((t) => t.compactEven),
                  d.layoutBindings[d.layout].compactDimension.sizeColumn
                ),
                a = M.max(
                  n.filter((t) => !t.compactEven),
                  d.layoutBindings[d.layout].compactDimension.sizeColumn
                );
              const o = 2 * Math.max(e, a),
                i = this.groupBy(
                  n,
                  (t) => t.row,
                  (t) => M.max(t, (t) => d.layoutBindings[d.layout].compactDimension.sizeRow(t) + d.compactMarginBetween(t))
                ),
                r = M.sum(i.map((t) => t[1]));
              n.forEach((t) => {
                (t.firstCompactNode = n[0]), t.firstCompact ? (t.flexCompactDim = [o + d.compactMarginPair(t), r - d.compactMarginBetween(t)]) : (t.flexCompactDim = [0, 0]);
              }),
    ...
    calculateCompactFlexPositions(t) {
      ...
      t.eachBefore((t) => {
        if (t.children) {
          const a = t.children.filter((t) => t.flexCompactDim),
            n = a[0];
          if (n) {
            a.forEach((t, e, a) => {
              0 == e && (n.x -= n.flexCompactDim[0] / 2), e & ((e % 2) - 1) ? (t.x = n.x + 0.25 * n.flexCompactDim[0] - d.compactMarginPair(t) / 4) : e && (t.x = n.x + 0.75 * n.flexCompactDim[0] + d.compactMarginPair(t) / 4);
            });
            var e = n.x + 0.5 * n.flexCompactDim[0];
            n.x = n.x + 0.25 * n.flexCompactDim[0] - d.compactMarginPair(n) / 4;
            const o = t.x - e;
            Math.abs(o) < 10 && a.forEach((t) => (t.x += o));
            const i = this.groupBy(
                a,
                (t) => t.row,
                (t) => M.max(t, (t) => d.layoutBindings[d.layout].compactDimension.sizeRow(t))
              ),
              r = M.cumsum(i.map((t) => t[1] + d.compactMarginBetween(t)));
            a.forEach((t, e) => {
              t.row ? (t.y = n.y + r[t.row - 1]) : (t.y = n.y);
            });
          }
        }
      });
    ...
    update({ x0: a, y0: n, x: o = 0, y: i = 0, width: r, height: d }) {
      ...
      const m = s.linksWrapper.selectAll('path.link').data(l, (t) => s.nodeId(t.data)),
        f = m
          .enter()
          .insert('path', 'g')
          .attr('class', 'link')
          .attr('d', (t) => {
            var e = { x: s.layoutBindings[s.layout].linkJoinX({ x: a, y: n, width: r, height: d }), y: s.layoutBindings[s.layout].linkJoinY({ x: a, y: n, width: r, height: d }) };
      ...
      y.attr('fill', 'none'),
        ...
        y
          ...
          .duration(s.duration)
          .attr('d', (t) => {
            var e = s.compact && t.flexCompactDim ? { x: s.layoutBindings[s.layout].compactLinkMidX(t, s), y: s.layoutBindings[s.layout].compactLinkMidY(t, s) } : { x: s.layoutBindings[s.layout].linkX(t), y: s.layoutBindings[s.layout].linkY(t) },
      ...
      m.exit()
        ...
        .duration(s.duration)
        .attr('d', (t) => {
          var e = { x: s.layoutBindings[s.layout].linkJoinX({ x: o, y: i, width: r, height: d }), y: s.layoutBindings[s.layout].linkJoinY({ x: o, y: i, width: r, height: d }) };
      ...
      const C = s.nodesWrapper.selectAll('g.node').data(e, ({ data: t }) => s.nodeId(t)),
        b = C.enter()
          .append('g')
          .attr('class', 'node')
          .attr('transform', (t) => {
          ...
          .attr('cursor', 'pointer')
          .on('click', (t, { data: e }) => {
            [...t.srcElement.classList].includes('node-button-foreign-object') || s.onNodeClick(t, s.nodeId(e));
          });
      b.patternify({ tag: 'rect', selector: 'node-rect', data: (t) => [t] });
      const k = b.merge(C).style('font', '12px sans-serif'),
        $ = k.patternify({ tag: 'foreignObject', selector: 'node-foreign-object', data: (t) => [t] }).style('overflow', 'visible');
      $.patternify({ tag: 'xhtml:div', selector: 'node-foreign-object-div', data: (t) => [t] }), this.restyleForeignObjectElements();
      const _ = b.patternify({ tag: 'g', selector: 'node-button-g', data: (t) => [t] }).on('click', (t, e) => this.onButtonClick(t, e));
      _.patternify({ tag: 'rect', selector: 'node-button-rect', data: (t) => [t] })
        .attr('opacity', 0)
        ...
        .attr('x', -20)
        .attr('y', -20);
      _.patternify({ tag: 'foreignObject', selector: 'node-button-foreign-object', data: (t) => [t] })
        .attr('width', 40)
        .attr('height', 40)
        .attr('x', -20)
        .attr('y', -20)
      ...
      k
        ...
        k.select('.node-button-foreign-object .node-button-div').html((t) => s.buttonContent({ node: t, state: s })),
        k
      ...
      C.exit()
        ...
        .duration(s.duration)
        .attr('transform', (t) => {
          return `translate(${s.layoutBindings[s.layout].nodeJoinX({ x: o, y: i, width: r, height: d })},${s.layoutBindings[s.layout].nodeJoinY({ x: o, y: i, width: r, height: d })})`;
        })
        .on('end', function () {
      ...
      E && ((u = E.data._centeredWithDescendants ? E.descendants().filter((t, e) => e < 7) : [E]), (E.data._centeredWithDescendants = null), (E.data._centered = null), this.fit({ animate: !0, scale: !1, nodes: u }));
    }
    isEdge() {
    ...
    hdiagonal(t, e, a) {
      var n = t.x,
        ...
        t = i - n < 0 ? -1 : 1,
        e = r - o < 0 ? -1 : 1;
      let l = Math.abs(i - n) / 2 < 35 ? Math.abs(i - n) / 2 : 35;
      l = Math.abs(r - o) / 2 < l ? Math.abs(r - o) / 2 : l;
      Math.abs(r - o), l;
      a = Math.abs(i - n) / 2 - l;
      return `
                  M ${d} ${s}
    ...
    diagonal(t, e, a) {
      var n = t.x,
        ...
        l = i - n < 0 ? -1 : 1,
        t = r - o < 0 ? -1 : 1;
      let h = Math.abs(i - n) / 2 < 35 ? Math.abs(i - n) / 2 : 35;
      h = Math.abs(r - o) / 2 < h ? Math.abs(r - o) / 2 : h;
      (e = Math.abs(r - o) / 2 - h), (a = Math.abs(i - n) - 2 * h);
      return `
                  M ${d} ${s}
                  L ${n} ${s}
                  L ${n} ${o}
    ...
    restyleForeignObjectElements() {
      ...
      n.svg
        ...
        n.svg
          ...
          .html(function (t, e, a) {
            return n.nodeContent.bind(this)(t, e, a, n);
          });
    }
    onButtonClick(t, e) {
      t.stopPropagation();
    ...
    setLayouts({ expandNodesFirst: t = !0 }) {
      ...
      (i.root = M.stratify()
        .id((t) => i.nodeId(t))
        .parentId((t) => i.parentNodeId(t))(i.data)),
        i.root.each((t, e, a) => {
          var n = i.nodeWidth(t),
            o = i.nodeHeight(t);
          Object.assign(t, { width: n, height: o });
        }),
        (i.root.x0 = 0),
        (i.root.y0 = 0),
        (i.allNodes = i.root.descendants()),
        i.allNodes.forEach((t) => {
          Object.assign(t.data, { _directSubordinates: t.children ? t.children.length : 0, _totalSubordinates: t.descendants().length - 1 });
        }),
        i.root.children && (t && i.root.children.forEach(this.expand), i.root.children.forEach((t) => this.collapse(t)), 0 == i.expandLevel && ((i.root._children = i.root.children), (i.root.children = null)), [i.root].forEach((t) => this.expandSomeNodes(t)));
    }
    collapse(t) {
    ...
    zoomTreeBounds({ x0: t, x1: e, y0: a, y1: n, params: o = { animate: !0, scale: !0 } }) {
      const { centerG: i, svgWidth: r, svgHeight: d, svg: s, zoomBehavior: l, duration: h, lastTransform: c } = this.getChartState();
      var g = Math.min(8, 0.9 / Math.max((e - t) / r, (n - a) / d));
      let u = M.zoomIdentity.translate(r / 2, d / 2);
      (u = u.scale(o.scale ? g : c.k)),
        (u = u.translate(-(t + e) / 2, -(a + n) / 2)),
        s
          .transition()
          .duration(o.animate ? h : 0)
          .call(l.transform, u),
        i
          .transition()
          .duration(o.animate ? h : 0)
    ...
    fit({ animate: t = !0, nodes: e, scale: a = !0 } = {}) {
      const n = this.getChartState(),
        { root: o } = n;
      var i = e || o.descendants(),
        r = M.min(i, (t) => t.x + n.layoutBindings[n.layout].nodeLeftX(t)),
        d = M.max(i, (t) => t.x + n.layoutBindings[n.layout].nodeRightX(t)),
        e = M.min(i, (t) => t.y + n.layoutBindings[n.layout].nodeTopY(t)),
        i = M.max(i, (t) => t.y + n.layoutBindings[n.layout].nodeBottomY(t));
      return this.zoomTreeBounds({ params: { animate: t, scale: a }, x0: r - 50, x1: d + 50, y0: e - 50, y1: i + 50 }), this;
    }
    setExpanded(e, t = !0) {
      const a = this.getChartState(),
        n = a.allNodes.filter(({ data: t }) => a.nodeId(t) == e)[0];
      return n ? (n.data._expanded = t) : console.log(`ORG CHART - ${t ? 'EXPAND' : 'COLLAPSE'} - Node with id (${e})  not found in the tree`), this;
    }
    setCentered(e) {
      const a = this.getChartState(),
        t = a.allNodes.filter((t) => a.nodeId(t.data) === e)[0];
    ...
    fullscreen(t) {
      const e = this.getChartState(),
        a = M.select(t || e.container).node();
      M.select(document).on('fullscreenchange.' + e.id, function (t) {
    ...
    exportImg({ full: a = !1, scale: n = 3, onLoad: o = (t) => t, save: i = !0 } = {}) {
      const r = this;
      const { svg: t, root: d } = this.getChartState();
      let e = 0;
      const s = t.selectAll('img');
      let l = s.size();
      const h = () => {
        JSON.parse(JSON.stringify(r.lastTransform()));
        var t = r.duration();
        a && r.fit();
        const { svg: e } = r.getChartState();
        setTimeout(
          (t) => {
            r.downloadImage({
              node: e.node(),
    ...
    exportSvg() {
      const { svg: t } = this.getChartState();
      return this.downloadImage({ node: t.node(), scale: 3, isSvg: !0 }), this;
    }
    expandAll() {
      const { allNodes: t } = this.getChartState();
    ...
    downloadImage({ node: t, scale: e = 2, isSvg: a = !1, save: n = !0, onAlreadySerialized: o = (t) => {}, onLoad: i = (t) => {} }) {
      const r = t;
      if (a) {
        var d = '<?xml version="1.0" standalone="no"?>\r\n' + (d = c(r));
        return h((d = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(d)), 'graph.svg'), void o();
      }
      const s = e,
      ...
      l.onload = function () {
        ...
        (t.width = e.width * s), (t.height = e.height * s);
        const a = t.getContext('2d');
        (a.fillStyle = '#FAFAFA'), a.fillRect(0, 0, e.width * s, e.height * s), a.drawImage(l, 0, 0, e.width * s, e.height * s);
        e = t.toDataURL('image/png');
        i && i(e), n && h(e, 'graph.png');
      };
      d = 'data:image/svg+xml; charset=utf8, ' + encodeURIComponent(c(r));
      function h(t, e) {
      ...
      function c(t) {
        ...
        const n = document.createTreeWalker(t, NodeFilter.SHOW_ELEMENT, null, !1);
        for (; n.nextNode(); ) for (const i of n.currentNode.attributes) i.value.includes(a) && (i.value = i.value.replace(a, '#'));
        ...
        return o.serializeToString(t);
      }
      o(), (l.src = d);
    }
    getTextWidth(t, { fontSize: e = 14, fontWeight: a = 400, defaultFont: n = 'Helvetice', ctx: o } = {}) {
      return (o.font = `${a || ''} ${e}px ${n} `), o.measureText(t).width;
    }
  }),
